...
Generates the fixed pseudorandom trial sequences used by triangles_imagery.py
(imagery task) and triangles_perception.py (perception task):

    training_trials.csv     (4 trials  - imagery training)
    imagery_trials.csv      (60 trials - imagery task)
    perception_trials.csv   (10 trials - perception task, no training)

Each trial has two independently counterbalanced factors:
    - cue / cued_image            : up-arrow or down-arrow, 50/50 split, no
                                     more than MAX_RUN identical cues in a
                                     row.
    - first_image / second_image  : the order of the two-stimulus preview
                                     shown at the start of every trial
                                     (up.png then down.png, or vice-versa),
                                     also 50/50 with the same run constraint.
...
def _shuffle_balanced_sequence(n, seed, labels, max_attempts=100000):
    """Shuffle-and-reject generator. Works fine for small n; for larger n
    (e.g. n=60 at MAX_RUN=2) the odds of a random full-sequence shuffle
    satisfying the run constraint collapse combinatorially, so this returns
    None and the caller falls back to _construct_balanced_sequence."""
    half = n // 2
...
def write_csv(filename, n, cue_seed, order_seed):
    cues   = generate_balanced_sequence(n, cue_seed,   ("↑", "↓"))
    orders = generate_balanced_sequence(n, order_seed, ("UD", "DU"))
