"""
triangles_perception.py -- Perception task

Run this as its own session, entirely separate from triangles_imagery.py
...
Design:
    ...
    task (up.png and down.png, fullscreen, back to back), in a
    counterbalanced order (see generate_trial_csvs.py). The cued image is
    then shown fullscreen during the "imagery" period instead of a blank
    screen; no vividness / time-to-imagine ratings are collected.
...
def main():
    ...
    log_file, et_log = make_log_paths("triangles_perception", subject_nr)
