"""
triangles_imagery.py -- Imagery task

Run this as its own session. The perception task lives entirely in
triangles_perception.py; the two scripts do not depend on each other.
Shared setup, timing, and trial-running logic live in
triangles_experiment_common.py.

Design:
    Every trial opens with a two-stimulus preview (up.png and down.png,
    fullscreen, back to back) before the imagery cue -- there is no
    separate block-intro step. The preview order is counterbalanced across
    trials (see generate_trial_csvs.py). The cue itself is a small up/down
    arrow shown in the center of the screen (instead of a letter), pointing
    to whichever of the two stimuli the participant should imagine.

    Training:  4 practice trials (no ET, not logged).
    Imagery:   60 trials. Break screen after every 10 trials (except the
               last).

See triangles_experiment_common.py for the full per-trial timing breakdown.
"""

from psychopy import core
from triangles_experiment_common import (
    draw_text,
    load_trials,
    make_log_paths,
...
def main():
    ...
    log_file, et_log = make_log_paths("triangles_imagery", subject_nr)
